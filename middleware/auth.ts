...
const verifyAdmin = (req: any, res: Response, next: any) => {
  ...
  try {
    ...
    if(decoded.user.role.role_name === 'Admin'){
      req.user = decoded;
    }else {
      return res.status(401).send('You do not have permission for this operation');
    }
  } catch (err) {
    basicLogger.error({
      controller: 'token verification',
...
export { verifyToken, verifyTokenForApi ,verifyAdmin};