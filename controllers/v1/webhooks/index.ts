...
const fetchShopifyOrderUsingWebhook = async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({ success: false, error: "Failed to fetch shopify products" });
  }
};
...
const sendOrderPlacementMessageToRenter = async (body: any) => {
  ...
  if (arrayLength > 1) {
    ...
    duration =
      line_items_array[0]?.name.split("/").length > 0 && line_items_array[0].name.split("/").length == 4
        ? line_items_array[0].name.split("/")[3]
        : line_items_array[0].name.split("/")[2];
